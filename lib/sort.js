...
const schelsort = (t, p) => {
  if (p.tfin != null) {
    return 9e9 - moment(p.tfin).unix()
  }
  if (p.tdue == null) {
    return (p.tini == null ? -9e9 : moment(p.tini).unix()) - t
  }
<<<<<<< HEAD
  return scheldist(t, p.tdue.unix())
=======
  return scheldist(t, moment(p.tdue).unix())
>>>>>>> moment-ize dates before using moment functions
}
